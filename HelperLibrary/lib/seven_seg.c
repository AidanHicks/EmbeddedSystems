...
unsigned char DIG_MAP[4] = {3,2,1,0}; // copy from 5C after testing
...
void seven_seg_scan(void){
        all_off(); LATD=0x00; Delay_us(BLANK_US); LATD=seg_for(segments[0]); enable_pos(0);
        Delay_us(SCAN_ON_US);
        all_off(); LATD=0x00; Delay_us(BLANK_US); LATD=seg_for(segments[1]); enable_pos(1);
        Delay_us(SCAN_ON_US);
        all_off(); LATD=0x00; Delay_us(BLANK_US); LATD=seg_for(segments[2]); enable_pos(2);
        Delay_us(SCAN_ON_US);
        all_off(); LATD=0x00; Delay_us(BLANK_US); LATD=seg_for(segments[3]); enable_pos(3);
        Delay_us(SCAN_ON_US);
}